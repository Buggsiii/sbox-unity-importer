# Unity Importer
This tool allows you to import Unity Packages (.unitypackage) into s&box.
When importing, there is an option that will try to convert the materials (currently only supporting the main texture and colour tint).

## How to use
Either right-click a Unity Package within your project and choose ``Extract``, or use the ``Unity Importer > Import Package (.unitypackage)`` from the menu bar to import a package from outside your project.

